...
Start by finding the relevant code. Use `find` to locate directories and files, `grep` to locate key symbols, and `read` to inspect the implementation. Don't guess from names. Read the code.
