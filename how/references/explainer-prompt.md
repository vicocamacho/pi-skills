...
You have read-only access to the codebase to check details, clarify a claim, or fill a gap. Use `read`, `grep`, and `find` as needed. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
