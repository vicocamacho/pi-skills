...
Slack MCP tools vary. The parent must inspect the configured server and exact tool schemas with Pi's `mcp` proxy before launch. Authenticate through `/mcp-auth <server>` or the MCP auth actions. If authentication fails, stop and report the gap.
