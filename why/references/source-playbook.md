...
Before launch, the parent must inspect each configured server with Pi's `mcp` proxy and copy the exact direct tool names and schemas into the investigator task. Tool names shown in these playbooks are examples and may need a server prefix. A category is available to subagents only when an executable agent profile includes `mcp:<server-name>` in its strict tool list.
